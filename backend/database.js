    ...
    if (err) {
      ...
      db.get(
        ...
        (err, result) => {
          ...
          if (result) {
            // 'log' table already exists, insert rows
            ...
            // 'log' table does not exist, create it and insert rows
            db.run(
              `
            CREATE TABLE log (
    ...
    db.run(
      ...
      (err) => {
        if (err) {
          ...
          var insertUser = "INSERT INTO user (username,  password) VALUES (?,?)";
    ...
    db.run(
      ...
      (err) => {
        if (err) {
          ...
          // Table just created, creating some rows
          var insertCategory = "INSERT INTO category (category_name) VALUES (?)";
